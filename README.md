...
**Backend:** This repo is **frontend only**. The unified API (HHD + Picker) runs from **`Desktop/Dev/Backend`**. Set `EXPO_PUBLIC_API_URL` or `app.json` → `extra.apiUrl` to `http://YOUR_IP:5000/api` so this app talks to that backend.
...
This is the **frontend** folder. The repo root has `frontend/` and `backend/`.
