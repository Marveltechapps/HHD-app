...
output() {
    ...
    case "$level" in
        "info")
            [ "$VERBOSE_MODE" = true ] && echo -e "${BLUE}ℹ️  $message${NC}"
            ;;
        "success")
...
prompt_user() {
    ...
    local prompt_text="${YELLOW}❓ $question${NC} [y/N]: "
    if [ "$default" = "y" ]; then
        prompt_text="${YELLOW}❓ $question${NC} [Y/n]: "
    fi
...
log_task() {
    ...
    case "$status" in
        ...
        "failed")
            ...
            ;;
        "warning")
            EXECUTED_TASKS+=("⚠️  $task")
            WARNINGS+=("$task: $details")
            ;;
        "skipped")
            EXECUTED_TASKS+=("⏭️  $task (skipped)")
            ;;
    esac
}
    ...
    if [ "$BUILD_MODE" = "local" ] && [ "$BUILD_VARIANT" = "release" ]; then
        ...
        if [ ! -f "$keystore_path" ]; then
            ...
            if [ "$VERBOSE_MODE" = true ]; then
                ...
                output "info" "ℹ️  Keystore not found (will use debug signing for release build)"
            fi
...
if [ "$JSON_MODE" = false ]; then
    ...
    echo -e "${CYAN}⚙️  Build profile: ${BUILD_PROFILE}${NC}"
    echo ""
fi
...
if [ "$JSON_MODE" = false ]; then
    ...
    output "info" "⏱️  Total build time: ${MINUTES}m ${SECONDS}s"
    echo ""
fi
...
if [ "$JSON_MODE" = false ]; then
    ...
    if [ ${#BUILD_TIMINGS_NAMES[@]} -gt 0 ]; then
        echo -e "${BLUE}⏱️  Timing Information:${NC}"
        echo ""
        local i=0
    ...
    if [ ${#WARNINGS[@]} -gt 0 ]; then
        echo -e "${YELLOW}⚠️  Warnings (${#WARNINGS[@]} total):${NC}"
        echo ""
    ...
    if [ ${#FAILED_TASKS[@]} -gt 0 ]; then
        echo -e "${RED}❌ Failed Tasks (${#FAILED_TASKS[@]} total):${NC}"
        echo ""
    ...
    if [ ${#FAILED_TASKS[@]} -eq 0 ]; then
        ...
        echo -e "${YELLOW}⚠️  Build completed with ${#FAILED_TASKS[@]} failed task(s)${NC}"
